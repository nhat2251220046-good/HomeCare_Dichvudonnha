...
export default function OrderDetailUser() {
  ...
  const [serviceReview, setServiceReview] = useState({ rating: 5, comment: "" });
  const [staffReview, setStaffReview] = useState({ rating: 5, comment: "" });
  ...
  const handleServiceReviewSubmit = async (e) => {
    ...
    try {
      ...
      // Refresh order data to show the new review
      const res = await axios.get(`http://localhost:5000/api/orders/${id}`);
      setOrder(res.data);
      setServiceReview({ rating: 5, comment: "" }); // Reset form
    } catch (err) {
  ...
  const handleStaffReviewSubmit = async (e) => {
    ...
    try {
      ...
      const res = await axios.get(`http://localhost:5000/api/orders/${id}`);
      setOrder(res.data);
      setStaffReview({ rating: 5, comment: "" }); // Reset form
    } catch (err) {
  ...
  const serviceReviewExists = Boolean(
    order?.review?.rating || order?.review?.comment?.trim()
  );
  const staffReviewExists = Boolean(
    order?.staffReview?.rating || order?.staffReview?.comment?.trim()
  );
  ...
  return (
    <div className="min-h-screen bg-gray-50 py-8 px-4">
      ...
      <div className="max-w-3xl mx-auto bg-white rounded-2xl shadow p-6">
        ...
        {/* Success Message for Canceled Orders */}
